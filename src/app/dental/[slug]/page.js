...
import { base_endpoint, appname, frontend_url, headerx, image_base_endpoint } from '../../../utils/constants'
...
const fetchDetail = async (slug) => {
  try {
    ...
    const response = await fetch(`${base_endpoint}/GeneralInformation/GetAllGenericServiceList?serviceType=1&userId=${slug}`, {
      method: "GET",
      headers: headerx,
      cache: "no-store"
    });
...
async function Page({ params }) {
  ...
  return (
    <>
      ...
      <AppBar leadingIcon={<FaArrowLeft className="h-5 w-5" />} title='Dental Detail' trailingComponents={
        <div className='flex'>

          <ProfileQR userId={data.userId} type={"Dental"}></ProfileQR>

          <FavouriteToggle isFill={data.isFavourite} userId={user?.id} id={data.userId} type={3} token={token}  ></FavouriteToggle>

          <ShareButton link={`${frontend_url}/dental/${data.userId}`}></ShareButton>
        </div>
      }></AppBar>

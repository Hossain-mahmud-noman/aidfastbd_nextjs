...
import BloodTabs from '../../../components/tabs/BloodTabs';
...
const fetchDetail = async (slug) => {
  try {
    ...
    const response = await fetch(`${base_endpoint}/GeneralWeb/GetAllBloodBankList?pageNumber=1&pageSize=1&bloodBankInformationId=${slug}`, {
      method: "GET",
      headers: headerx,
      cache: "no-store"
  ...
  return (
    <>
      ...
      <AppBar leadingIcon={<FaArrowLeft className="h-5 w-5" />} title='Blood Bank Detail' trailingComponents={
        <div className='flex'>
          <ProfileQR  userId={data.userId} type={"BloodBank"}></ProfileQR>
          <FavouriteToggle isFill={data.isFavourite} userId={user?.id} id={data.userId} type={3} token={token}  ></FavouriteToggle>

          <ShareButton link={`${frontend_url}/blood/${data.userId}`}></ShareButton>
        </div>
      }></AppBar>
...
// https://api.aidfastbd.com/api/GeneralWeb/GetAllBloodBankList?pageNumber=1&pageSize=1&bloodBankInformationId=d0b88596-dd49-4447-705f-08dbef0a5729

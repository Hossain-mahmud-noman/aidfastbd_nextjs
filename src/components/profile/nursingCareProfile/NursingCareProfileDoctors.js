...
function Nurses({ data, token, id, getProfileData }) {
  ...
  const saveDoctor = async (values) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Nurse ${isEditMode ? 'updated' : 'added'} successfully!`);
      } else {
        toast.error(`Failed to ${isEditMode ? 'update' : 'add'} Nurse.`);
      }
    } catch (error) {
      console.error("Error saving doctor:", error);
      toast.error(`Something went wrong while ${isEditMode ? 'updating' : 'adding'} Nurse.`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const removeDoctor = async (doctorId) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "Do you want to remove this Nurse?",
      icon: "warning",
    ...
    try {
      ...
      toast.error("Something went wrong while removing Nurse.");
    }
  };
  ...
  return (
    <div className="bg-white shadow-custom-light rounded-lg w-full max-w-3xl mx-auto p-6">
      <h1 className="text-lg font-bold mb-4">
        Add Nurse profile of your Nursing Care Center
      </h1>
      ...
      <Modal
        title={isEditMode ? "Edit Nurse" : "Add New Nurse"}
        open={modalVisible}
        onCancel={closeModal}
        footer={null}
        width={600}

...
function AmbulanceProfileBasic({ data, user, token }) {
  ...
  const handleData = async () => {
    ...
    try {
      const url = `${base_endpoint}/GeneralInformation/AmbulanceRegistration`;
  ...
  return (
    <div className="bg-white shadow-md rounded-lg w-full max-w-lg p-6">
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Add a cover picture of your Ambulance
        </label>
      ...
      {/* Input Fields */}
      <InputField required label="Ambulance Name (English)" placeholder="Enter Ambulance Name" value={name} onChange={setName} />
      <InputField required label="Ambulance Name (Bangla)" placeholder="Enter Ambulance Name Bangla" value={nameBn} onChange={setNameBn} />
      ...
      <InputField label="Ambulance Notice" placeholder="Enter your notice" value={notice} onChange={setNotice} />
...
export default AmbulanceProfileBasic;

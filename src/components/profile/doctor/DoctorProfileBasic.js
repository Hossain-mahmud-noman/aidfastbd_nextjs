...
function DoctorProfileBasic({ data, user, token }) {
    ...
    const fetchSpecialities = async () => {
        try {
            const res = await fetch("https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=Speciality");
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetch("https://api.aidfastbd.com/api/GeneralInformation/DoctorInformationSave", {
                method: "POST",
                headers: {
    ...
    return (
        <div className="bg-white shadow-md rounded-lg w-full max-w-lg p-6">
            <h2 className="text-xs font-semibold text-gray-700 mb-4">Add a Profile Picture</h2>
            ...
            {/* Name Section */}
            <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">নাম বাংলা ভাষা</label>
                <div className="flex gap-2">
                    <InputField
            ...
            <p className="text-xm text-red-500 mb-4">** Name should be same as BMDC registration certificate. After submission, name cannot be changed or edited **</p>
            ...
            <Dropdown
                label="Degree"
                options={[
                    { value: "41C6856B-4531-4C16-BB6D-08DB62033D21".toLowerCase(), text: "MBBS" },
                    { value: "2BCAE9D2-C382-4405-BA41-F926E20D7306".toLowerCase(), text: "BDS" },
                    { value: "9B4B6BA9-FED7-4655-B433-537490898290".toLowerCase(), text: "MD (Rusia)" }
            ...
            <Dropdown
                label="BCS"
                options={[
                    { value: "3DC212B3-2D0E-4FA3-B359-08DB62041BE3".toLowerCase(), text: "Health" },
                    { value: "5998B807-487C-4800-858F-F2200D35FA95".toLowerCase(), text: "No" },
            ...
            <Dropdown
                ...
                options={[
                    { value: "98B91DDC-4523-499D-8E89-884E750BB588".toLowerCase(), text: "Temporary" },
                    { value: "4C1BF16C-C81F-43CB-F578-08DB6203C497".toLowerCase(), text: "Permanent" },
            ...
            />
            <p className="text-xm text-red-500 mb-4">** If you want to provide emergency service add an emergency contact number, otherwise skip **</p>
            ...
            />
            <hr className="my-6" />
            <p className="text-xl text-blue-500 mb-4">Information for our database and authentication: <br></br>(All information give below will be kept private. Only you can see this.)</p>

            {/* Gender Dropdown */}
            <Dropdown
                label="Gender"
                options={[
                    { value: "6AEA5873-BA76-4C60-A3D4-9A02530D5705".toLowerCase(), text: "Female" },
                    { value: "5E579E98-EA2A-4196-B9F2-08DB61E8F0B1".toLowerCase(), text: "Male" }
                ]}
            ...
            {/* Other Input Fields */}
            <InputField label="NID Number" value={nidNumber} onChange={(e) => setNidNumber(e.target.value)} placeholder="Enter NID Number" />
            <InputField label="Personal Phone Number" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="Enter Phone Number" />
            <InputField label="Medical College" value={medicalCollege} onChange={(e) => setMedicalCollege(e.target.value)} placeholder="Enter your medical college name" />
            <InputField label="Medical College Batch" value={batch} onChange={(e) => setBatch(e.target.value)} placeholder="Enter your batch" />
            <InputField label="Medical College Session" value={session} onChange={(e) => setSession(e.target.value)} placeholder="Enter your session" />

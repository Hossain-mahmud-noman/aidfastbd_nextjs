...
function NursingCareprofile({ token, user }) {
  ...
  const tabs = [
    ...
    { label: 'Info', content: <NursingCareProfileInfo data={profileData?.genericServiceInfos} user={user} token={token}/> },
    { label: 'Doctors', content: <NursingCareProfileDoctors data={profileData?.genericServiceDoctors} user={user} token={token}/> },

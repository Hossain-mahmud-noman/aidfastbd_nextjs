...
function CareGiver({ data, token, id, getProfileData }) {
  ...
  const saveDoctor = async (values) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`CareGiver ${isEditMode ? 'updated' : 'added'} successfully!`);
      } else {
        toast.error(`Failed to ${isEditMode ? 'update' : 'add'} CareGiver.`);
      }
    } catch (error) {
      console.error("Error saving doctor:", error);
      toast.error(`Something went wrong while ${isEditMode ? 'updating' : 'adding'} CareGiver.`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const removeDoctor = async (doctorId) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "Do you want to remove this CareGiver?",
      icon: "warning",
    ...
    try {
      ...
      toast.error("Something went wrong while removing CareGiver.");
    }
  };
  ...
  return (
    <div className="bg-white shadow-custom-light rounded-lg w-full max-w-3xl mx-auto p-6">
      <h1 className="text-lg font-bold mb-4">
        Add CareGiver profile of your Nursing Care Center
      </h1>
      ...
      <Modal
        title={isEditMode ? "Edit CareGiver" : "Add New CareGiver"}
        open={modalVisible}
        onCancel={closeModal}
        footer={null}
        width={600}

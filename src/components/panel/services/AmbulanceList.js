...
} from '../../../redux/features/panel/panel-ambulanceSlice';
...
export const fetchDatas = async (dispatch, { page, search_query, slug = false }) => {
  ...
  try {
    ...
    const res = await axios.get('https://api2.aidfastbd.com/ambulance/', {
      params
    });
      ...
      {loading ? (
        ...
        <>
          <div className="grid md:grid-cols-3 gap-4 mb-3">
            {data.map((doc) => (
              <AmbulanceCard key={doc.id} service={doc} onSave={(e) => {

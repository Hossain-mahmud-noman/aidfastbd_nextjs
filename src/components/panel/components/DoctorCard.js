...
export default function DoctorCard({ service, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${panel_base}/doctor/${serviceData.id}/meta-update`,
        {
    ...
    <div>
      ...
      >
        <Image
          ...
          src={
            service.imageUrl
              ? `https://api.aidfastbd.com/${service.imageUrl}`
              : '/default-doctor.png'
          }
          alt={service.name}
      ...
      {serviceData.slug &&  <div
        onClick={() => {
          window.open("https://aidfastbd.com/doctor/d/" +serviceData.slug, "_blank");
        }}

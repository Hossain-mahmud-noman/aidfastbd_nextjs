...
const SearchDrug = () => {
    ...
    const fetchDropdownList = async () => {
      try {
        const response = await fetch(
          'https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=Drug'
        );
        if (response.ok) {
  ...
  const fetchResults = async (filterValues) => {
    ...
    try {
      const response = await fetch(
        `https://api.aidfastbd.com/api/GeneralInformation/GetAllGenericServiceList?serviceType=2${query}`
      );
      const data = await response.json();
  ...
  // Handle filter modal form submission
    ...
    <>
      ...
      {hasSearched && (
        <div className="mb-10 px-3">
          ...
          {results.length === 0 ? (
            <p className="text-center text-gray-500">{i18n.t('no_drug_de_addiction_found')}</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {results.map((item, idx) => (
                <ServiceCard key={idx} slug="drug-de-addiction" data={item} />
              ))}
            </div>
          )}
        </div>

...
import { setName, setRating, setRank, setEmergency, setNameOptions } from '../../redux/features/dentalSlice';
...
const SearchDental = ({title=null}) => {
  ...
  const { nameOptions, name, rating, emergency, rank } = useSelector((state) => state.dental);

  const fetchDropdownList = async () => {
    try {
      const response = await
        fetch(`https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=DentalClinic`,);
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-[10000]">
          <div className="bg-white rounded-lg w-11/12 max-w-md p-6 shadow-lg overflow-y-auto max-h-[90vh]">
            ...
            <form className="space-y-4" onSubmit={(e) => {
              e.preventDefault();
              getDentalList({ dispatch: dispatch,isSearch:true, emergency: emergency?.value,dental: name?.value, rating: rating?.value, rank: rank?.value});
              togglePopup();
            }}>
              <SearchableDropdown
                onSelect={name}

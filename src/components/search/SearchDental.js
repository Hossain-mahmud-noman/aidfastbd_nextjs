...
export const SearchableDropdown = ({ label, options, dispatch, onSelect }) => {
  ...
  return (
    <div className="relative">
      ...
      <div className="border border-gray-300 rounded-lg px-3 py-2 flex items-center justify-between cursor-pointer focus:border-blue-500">
        <span onClick={toggleDropdown} style={{ width: "100%" }}>
...
const SearchDental = ({ title = null }) => {
  ...
  const { nameOptions, name, rating, emergency, rank } = useSelector(
    (state) => state.dental
  );
  ...
  const fetchDropdownList = async () => {
    try {
      const response = await fetch(
        `https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=DentalClinic`
      );
  ...
  return (
    <>
      ...
      >
        ...
        <button className="absolute right-0 mr-2 bg-blue-500 text-white p-2 rounded-lg hover:bg-blue-600 transition-colors duration-300 transform hover:scale-105">
...
export const DentalSearchByName = ({ title = null }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  ...
  const dispatch = useDispatch();
  const { name, nameOptions, rating, emergency, rank, doctorName } = useSelector(
    (state) => state.dental
  );
  ...
  const fetchDropdownList = async () => {
    try {
      const response = await fetch(
        `https://api.aidfastbd.com/api/Generic/SearchAllGenericService?serviceType=1`
      );
      if (response.status === 200) {
        const data = await response.json();
        dispatch(setNameOptions(data));
          ...
          <div className="bg-white rounded-lg w-11/12 max-w-md p-6 shadow-lg overflow-y-auto max-h-[90vh]">
            ...
            >
              ...
              <input
                ...
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full px-4 py-3 rounded-lg border-2 border-gray-200 focus:border-blue-500 focus:outline-none transition-all duration-300 shadow-lg"
                placeholder="Name"
              />
              <button
                type="submit"

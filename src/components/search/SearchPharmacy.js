  ...
  return (
    <div className="relative">
      ...
      <div className="border border-gray-300 rounded-lg px-3 py-2 flex items-center justify-between cursor-pointer">
        <span onClick={toggleDropdown} style={{ "inline-size": "100%" }}>{onSelect?.text || `Select ${label.toLowerCase()}`}</span>
...
const SearchPharmacy = () => {
  ...
  const fetchDropdownList = async () => {
    try {
      const response = await fetch(`https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=Pharmacy`);
      if (response.ok) {
  ...
  const fetchPharmacyList = async () => {
    ...
    if (name) query += `&pharmacyInformationId=${name.value}`;
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-[10000]">
          <div className="bg-white rounded-lg w-11/12 max-w-md p-6 shadow-lg overflow-y-auto max-h-[90vh]">
            ...
            <h2 className="text-center text-lg font-semibold mb-2">Search Pharmacy</h2>

...
const SearchPharmacy = () => {
  ...
  useEffect(() => {
    fetch(
      "https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=Pharmacy"
    )
      .then((res) => res.json())
      .then((data) => setNameOptions(data))
      .catch((err) => console.error("Pharmacy dropdown fetch error", err));
  }, []);

  const handleSearch = async (customFilters = filters) => {
    const query = new URLSearchParams();
    if (customFilters.name?.value)
      query.append("pharmacyInformationId", customFilters.name.value);
    ...
    try {
      const res = await fetch(
        `${base_endpoint}/GeneralWeb/GetAllPharmacyList?pageNumber=1&pageSize=20&${query}`
      );

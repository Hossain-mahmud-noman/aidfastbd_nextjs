...
const SearchAmbulance = () => {
  ...
  useEffect(() => {
    fetch(`${base_endpoint}/Dropdown/GetDropDownList?type=Ambulance`)
      .then((res) => res.json())
      .then((data) => setNameOptions(data))
      .catch((err) => console.error("Ambulance dropdown fetch error", err));
  }, []);

  const handleSearch = async (customFilters = filters) => {
    const query = new URLSearchParams();
    if (customFilters.name?.value)
      query.append("ambulanceInformationId", customFilters.name.value);
    ...
    try {
      ...
      console.error("Ambulance search error", err);
    }
  };

...
import AmbulanceCard from "../AmbulanceCard"; // make sure this exists
    ...
    <div className="relative">
      ...
      <div className="border border-gray-300 rounded-lg px-3 py-2 flex items-center justify-between cursor-pointer">
        ...
        <span className="text-gray-500">
          {onSelect ? <MdCancel onClick={() => handleOptionClick(null)} /> : <FaAngleDown />}
        </span>
      </div>
      {isOpen && (
...
const SearchAmbulance = () => {
  ...
  const fetchDropdownList = async () => {
    try {
      const response = await fetch(`https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=Ambulance`);
      if (response.ok) {
  ...
  const fetchAmbulanceList = async () => {
    ...
    if (rating) query += `&ratngs=${rating.value}`;
    if (name) query += `&ambulanceInformationId=${name.value}`;

    try {
      const response = await fetch(`https://api.aidfastbd.com/api/GeneralWeb/GetAllAmbulanceList${query}`);
      const data = await response.json();
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-[10000]">
          <div className="bg-white rounded-lg w-11/12 max-w-md p-6 shadow-lg overflow-y-auto max-h-[90vh]">
            ...
            <h2 className="text-center text-lg font-semibold mb-2">Search Ambulance</h2>

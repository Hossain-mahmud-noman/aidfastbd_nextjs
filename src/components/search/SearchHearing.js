...
const SearchHearing = () => {
  ...
  const fetchDropdownList = async () => {
    try {
      const response = await fetch(`https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=Hearing`);
      if (response.ok) {
  ...
  const fetchBloodList = async () => {
    ...
    try {
      const response = await fetch(`https://api.aidfastbd.com/api/GeneralInformation/GetAllGenericServiceList?serviceType=4${query}`);
      const data = await response.json();

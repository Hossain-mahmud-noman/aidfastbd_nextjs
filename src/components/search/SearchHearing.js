...
const SearchHearing = () => {
    ...
    const fetchDropdownList = async () => {
      try {
        const response = await fetch(
          'https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=Hearing'
        );
        if (response.ok) {
  ...
  const fetchResults = async (filterValues) => {
    ...
    try {
      const response = await fetch(
        `https://api.aidfastbd.com/api/GeneralInformation/GetAllGenericServiceList?serviceType=4${query}`
      );
      const data = await response.json();
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2 md:gap-3 lg:gap-6 w-full px-3 mt-3">
        ...
        >
          <Image src="/filter.png" width={40} height={40} alt={i18n.t('Filter')} />
      ...
      {isFilterOpen && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-[10000]">
          <div className="bg-white rounded-lg w-11/12 max-w-md p-6 shadow-lg overflow-y-auto max-h-[90vh]">
            ...
            <h2 className="text-center text-lg font-semibold mb-2">{i18n.t('search_hearing_care_center')}</h2>
            ...
            <form className="space-y-4" onSubmit={handleSearchSubmit}>
              <SearchableDropdown
                label={i18n.t('Rating')}
                options={[
                  { value: 1, text: i18n.t('1 Star') },
                  { value: 2, text: i18n.t('2 Star') },
                  { value: 3, text: i18n.t('3 Star') },
                  { value: 4, text: i18n.t('4 Star') },
                  { value: 5, text: i18n.t('5 Star') },
                ]}
              ...
              <SearchableDropdown
                ...
                options={[
                  { value: 1, text: i18n.t('Rank 1') },
                  { value: 2, text: i18n.t('Rank 2') },
                  { value: 3, text: i18n.t('Rank 3') },
                  { value: 4, text: i18n.t('Rank 4') },
                ]}
                value={filters.rank}
      ...
      {hasSearched && (
        <div className="mb-10 px-3">
          ...
          {results.length === 0 ? (
            <p className="text-center text-gray-500">{i18n.t('no_hearing_care_center_found')}</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              {results.map((item, idx) => (
                <ServiceCard key={`${item.id ?? idx}`} slug="hearing-care-center" data={item} />
              ))}
            </div>
          )}
        </div>

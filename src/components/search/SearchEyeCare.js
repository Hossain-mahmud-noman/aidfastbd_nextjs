...
const SearchEyeCare = () => {
    ...
    const fetchDropdownList = async () => {
      try {
        const response = await fetch(
          'https://api.aidfastbd.com/api/Dropdown/GetDropDownList?type=EyeCare'
        );
        if (response.ok) {
  ...
  const fetchResults = async (filterValues) => {
    ...
    try {
      const response = await fetch(
        `https://api.aidfastbd.com/api/GeneralInformation/GetAllGenericServiceList?serviceType=5${query}`
      );
      const data = await response.json();
    ...
    <>
      ...
      {isFilterOpen && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-[10000]">
          <div className="bg-white rounded-lg w-11/12 max-w-md p-6 shadow-lg overflow-y-auto max-h-[90vh]">
            ...
            <h2 className="text-center text-lg font-semibold mb-2">{i18n.t("Search Eye Care Center")}</h2>
      ...
      {hasSearched && (
        <div className="mb-10 px-3">
          ...
          {results.length === 0 ? (
            <p className="text-center text-gray-500">{i18n.t('no_eye_care_center_found')}</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              {results.map((item, idx) => (
                <ServiceCard key={`${item.id ?? idx}`} slug="eye-care-center" data={item} />
              ))}
            </div>
          )}
        </div>

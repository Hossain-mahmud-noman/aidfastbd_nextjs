...
function AppointmentBooking({ id, user, token, chambers }) {
    ...
    const handleChange = (e) => {
        ...
        if (name === "selectedChamber") {
            // Reset date and slot selection if chamber changes
        ...
        if (name === "appointmentDate") {
            ...
            // Ensure a chamber is selected before filtering slots
            const chamberId = formData.selectedChamber;
            const chamber = chambers.find((ch) => ch.id === chamberId);

            if (chamber) {
                // Get all slots for the selected chamber
                ...
                // Filter slots based on the selected date's dayName
            ...
            // Reset slot selection if the date changes

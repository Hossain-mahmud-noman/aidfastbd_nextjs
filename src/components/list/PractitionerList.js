...
const PractitionerList = () => {
  ...
  const fetchAmbulances = async (pageNumber) => {
    try {
      ...
      const res = await axios.get(
        `https://api.aidfastbd.com/api/GeneralInformation/GetAllGenericServiceList?serviceType=8&pageNumber=${pageNumber}&lat=${lat}&lon=${lon}`
      );
      ...
      console.error('Failed to fetch general practitioner data:', error);
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handlePageChange = (newPage) => {
    if (newPage > 0 && newPage <= totalPages) {
      ...
      router.push(`/practitioner?${newSearchParams.toString()}`);
    }
  };
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-4">General Practitioner List</h1>
        ...
        <>
          {data.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {data.map((d, index) => (
                <ServiceCard key={index} data={d} slug='practitioner' />
              ))}
            </div>
          )}
        </>

...
const NursingCareList = () => {
  ...
  const fetchNursing = async (pageNumber) => {
    try {
      setLoading(true);
  ...
  const handlePageChange = (newPage) => {
    if (newPage > 0 && newPage <= totalPages) {
      ...
      router.push(`/nursing-home-care?${newSearchParams.toString()}`);
    }
  };
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-4">Nursing Care Center List</h1>

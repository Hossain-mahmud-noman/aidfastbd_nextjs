...
import NursingHomeCareCard from '../NursingHomeCareCard';
import { nursingHomeCareList } from '../../utils/func';
import { FaSpinner } from "react-icons/fa";

function NursingHomeCareList({ location, nextPage }) {
  ...
  const handleObserver = (entities) => {
    ...
    if (target.isIntersecting && !loading && nextPage !== -1) {
      nursingHomeCareList({ dispatch, lat: localStorage.getItem("lat"), lon: localStorage.getItem("lon"), page: page })
    }
  };
  useEffect(() => {
    const options = {
  ...
  return (
    ...
    <>
      {loading == false && data.length == 0 ? <div className='h-[300px] w-full flex items-center justify-center text-2xl'>No data available</div> : <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        {data.map((d) => (
          <NursingHomeCareCard key={d.id} data={d} />
        ))}
      </div>}
      ...
      >
        {loading && (
          <div className="flex items-center space-x-2">
            ...
            <span className="text-gray-600">Loading nursing care centers...</span>
          </div>
        )}
      </div>
    </>
...
export default NursingHomeCareList
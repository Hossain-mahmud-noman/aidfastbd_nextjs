...
const DoctorHomeVisitList = () => {
  ...
  const fetchDoctorHomeVisits = async (pageNumber) => {
    try {
      setLoading(true);
      ...
      console.error('Failed to fetch doctor home visit data:', error);
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    fetchDoctorHomeVisits(pageFromUrl);
  }, [searchParams]);
  ...
  const handlePageChange = (newPage) => {
    if (newPage > 0 && newPage <= totalPages) {
      ...
      router.push(`/doctor-home-visit?${newSearchParams.toString()}`);
    }
  };

...
import { getPhysiotherapyCenterList } from '../../utils/func';
import { FaSpinner } from "react-icons/fa";
import PhysiotherapyCenterCard from '../PhysiotherapyCenterCard';

function PhysiotherapyCenterList({ location, nextPage }) {
    ...
    const handleObserver = (entities) => {
        ...
        if (target.isIntersecting && !loading && nextPage !== -1) {
            getPhysiotherapyCenterList({ dispatch, lat: localStorage.getItem("lat"), lon: localStorage.getItem("lon"), page: page })
        }
    };
    useEffect(() => {
        const options = {
        ...
        <>
            {loading == false && data.length == 0 ? <div className='h-[300px] w-full flex items-center justify-center text-2xl'>No data available</div> : <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                {data.map((d) => (
                    ...
                    <PhysiotherapyCenterCard key={d.id} data={d} />
                ))}
            </div>}
            <div
                ref={loader}
...
export default PhysiotherapyCenterList
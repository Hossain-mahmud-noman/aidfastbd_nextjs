...
const CareGiverList = () => {
  ...
  const fetchCareGivers = async (pageNumber) => {
    try {
      setLoading(true);
      const res = await axios.get(
        `https://api.aidfastbd.com/api/GeneralInformation/GetAllGenericServiceList?serviceType=9&pageNumber=${pageNumber}&lat=${lat}&lon=${lon}`
      );
      ...
      console.error('Failed to fetch care giver data:', error);
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handlePageChange = (newPage) => {
    if (newPage > 0 && newPage <= totalPages) {
      ...
      router.push(`/care-giver-center?${newSearchParams.toString()}`);
    }
  };
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-4">Care Giver List</h1>
        ...
        <>
          {data.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 lg:gap-4 xl:gap-5">
              {data.map((d, index) => (
                <ServiceCard key={index} data={d} slug='care-giver-center' />
              ))}
            </div>
          )}
        </>

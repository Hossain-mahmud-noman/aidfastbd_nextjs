  ...
  return (
    <>
      ...
      <div className="p-4 mb-[70px]">
          ...
          <div className='container mx-auto'>
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              ...
              {data?.genericServiceDoctors?.map((e, index) => {
                ...
                return (
                  <DoctorCard id={e?.doctorUserId} lat={data?.latitude} lon={data?.longitude} key={`doctor_${index}`} doctor={e} />
                );
              })}
            </div>
          </div>
              ...
              return <div key={service?.id} className="relative bg-purple-100 p-4 rounded-lg shadow-md mb-3">
                ...
                <div className="flex mt-2">
                  {service?.imgList?.map((img, index) => (
                    <Image key={index} width={100} height={100} src={img.imgUrl} alt="Service" className="w-[100px] h-[100px] mr-1 rounded" />
                  ))}
                </div>
              </div>;
            })}
        ...
        {activeTab === 'Review' && (
          <ReviewList reviews={data?.genericServiceReview} averageRating={data?.averageRating} totalRatings={data?.totalRating}></ReviewList>
        )}
      </div>
    </>
  )
